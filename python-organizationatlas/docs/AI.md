...
**OrganizationAtlas** is a Python library for organization information lookup and enrichment. It provides a unified interface to multiple organization data providers (INSEE, INPI, Entdatagouv, etc.) using ProviderKit for provider management.
...
1. **Search organizations** with multiple data providers:
   ...
   - Get organization by reference ID (SIREN, SIRET, RNA, etc.)
   - Get organization documents, events, officers, and beneficial owners
...
├── src/organizationatlas/        # Main package
...
- `src/organizationatlas/providers/`: Organization data provider implementations
- `src/organizationatlas/commands/`: Command infrastructure for CLI system
...
Providers must inherit from `OrganizationAtlasProvider`:

```python
from organizationatlas.providers import OrganizationAtlasProvider

class MyProvider(OrganizationAtlasProvider):
    name = "my_provider"
    ...
    services = ["search_organization", "search_organization_by_reference", "get_organization_documents", "get_organization_events"]
```

### Required Services

All providers must implement:
- `search_organization(query: str, **kwargs)`: Search for organizations by query string
- `search_organization_by_reference(code: str, **kwargs)`: Get organization by reference ID (SIREN, SIRET, RNA, etc.)
- `get_organization_documents(code: str, **kwargs)`: Get organization documents (if supported)
- `get_organization_events(code: str, **kwargs)`: Get organization events (if supported)
- `get_organization_officers(code: str, **kwargs)`: Get organization officers (if supported)
- `get_ultimate_beneficial_owners(code: str, **kwargs)`: Get ultimate beneficial owners (if supported)

### Organization Format Standardization

- All providers must return organizations in the standardized format defined by `ORGANIZATIONATLAS_SEARCH_COMPANY_FIELDS`
- Map provider's native response format to standard organization format
- Use `None` or empty strings for missing optional fields
- Never omit required fields
- Store organization data with appropriate types and precision
...
OrganizationAtlas discovers commands from:
...
- [ ] Providers inherit from OrganizationAtlasProvider correctly
- [ ] Providers implement all required services
- [ ] Organization format follows standardization (ORGANIZATIONATLAS_SEARCH_COMPANY_FIELDS)
