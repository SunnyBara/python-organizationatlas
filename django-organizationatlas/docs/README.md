...
### OrganizationAtlas-Specific Guidelines

- **Provider development**: All providers must inherit from `OrganizationAtlasProvider` and implement required services
- **Organization format**: Always use the standardized organization format defined in `ORGANIZATIONATLAS_SEARCH_COMPANY_FIELDS`
...
- [ ] Inherit from `OrganizationAtlasProvider`
- [ ] Define `name`, `display_name`, and `description`
- [ ] Set `required_packages` if needed
- [ ] Configure `config_keys` and `config_defaults`
- [ ] Implement `search_organization()` method
- [ ] Implement `search_organization_by_reference()` method
- [ ] Implement `get_organization_documents()` method (if supported)
- [ ] Implement `get_organization_events()` method (if supported)
- [ ] Implement `get_organization_officers()` method (if supported)
- [ ] Implement `get_ultimate_beneficial_owners()` method (if supported)
- [ ] Map provider response to standard organization format
