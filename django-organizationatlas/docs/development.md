...
- **Django models**: Define Django models for organization data (Organization, OrganizationData, OrganizationDocument, OrganizationEvent)
- **Django admin**: Provide Django admin interfaces for organization models
- **OrganizationAtlas integration**: Use OrganizationAtlas providers for data lookup and enrichment
...
- **Consistent fields**: All providers must return organizations in the standardized format defined by `ORGANIZATIONATLAS_SEARCH_COMPANY_FIELDS`
- **Field mapping**: Each provider should map its native response format to the standard organization format
- **Missing fields**: Use `None` or empty strings for missing optional fields, never omit required fields
- **Data types**: Store organization data with appropriate types and precision
